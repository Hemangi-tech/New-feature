...
function App() {
  ...
  const handleVote = async (questionId) => {
    ...
    try {
      const result = await forumService.voteQuestion(questionId, userEnrollment);
  ...
  const handleUnvote = async (questionId) => {
    ...
    try {
      ...
      setUserVotedQuestions(userVotedQuestions.filter((id) => id !== questionId));
      await loadQuestions();
    } catch (err) {
      ...
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Peer Q&A Discussion Forum</h1>
              <p className="text-sm text-gray-600 mt-1">Study Sphere - Collaborative Learning Platform</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <footer className="bg-white border-t border-gray-200 mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <p className="text-center text-sm text-gray-600">
            Study Sphere - Peer Q&A Discussion Forum | Open Access Learning Platform
          </p>
        </div>
      </footer>
    </div>
